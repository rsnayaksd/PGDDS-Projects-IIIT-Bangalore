...
# Store the movie name in variable named top_2_movie
  ...
  # Function to get nth top movie
  top_movie_decision2 <- function(p_bollywood, movie_rank){
    # Get new bollywood dataframe in decreasing order of Tcollection
    new_bollywood <- p_bollywood[order(p_bollywood$Tcollection,decreasing = TRUE),]
    return (new_bollywood[movie_rank,1])
  } 
  # calling the above function to get second top movie name
  top_2_movie <- top_movie_decision2(bollywood,2) 
...
# Store the names of 4 movies in same sequence in movie_result vector
	...
	movie_result <- sapply(bollywood[,4:7], function(collectionCategory) bollywood[which.max(collectionCategory),1])
	
#Another way to call
#movie_result <- sapply(bollywood[,4:7], function(collectionCategory) bollywood[which.max(collectionCategory),]$Movie)
